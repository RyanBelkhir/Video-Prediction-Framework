...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, temb=None):
        ...
        if temb is not None and self.conditional:
            h += self.dense(temb).unsqueeze(-1).unsqueeze(-1) # TEST WITH MULTIPLICATION
...
class UNet(nn.Module):
    def __init__(self, config):
        ...
        self.nonlinearity = Swish()
        self.out = nn.Conv2d(ch, n_channels*(num_frames + num_frames_cond) if output_all_frames else n_channels*num_frames, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x, y=None, cond=None):
        ...
        if self.output_all_frames and cond is not None:
...
class UNet_DDPM(nn.Module):
    def __init__(self, version="DDPM", sigma_dist="linear", sigma_max=0.02, sigma_min=0.0001, num_scales=1000, gamma=False, noise_in_cond=False):
        super().__init__()
        ...
        self.unet = UNet(version)

        self.schedule = sigma_dist
        if self.schedule == 'linear':
            self.register_buffer('betas', get_sigmas(sigma_max, sigma_min, num_scales))   # large to small, doesn't match paper, match code instead
            ...
            self.register_buffer('alphas', get_sigmas(sigma_max, sigma_min, num_scales))  # large to small, doesn't match paper, match code instead
            self.register_buffer('alphas_prev', torch.cat([self.alphas[1:], torch.tensor([1.0]).to(self.alphas)]))
